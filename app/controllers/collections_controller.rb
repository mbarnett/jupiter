class CollectionsController < ApplicationController
  ...
  def new
    @collection = Collection.new_cached_remote_object(community_id: params[:community_id])
    authorize @collection
  ...
  def create
    ...
    authorize @collection
    @collection.flush_cache_and_perform_remote_write(&:save!)
  ...
  def update
    @collection = Collection.find(params[:id])
    authorize @collection
    @collection.flush_cache_and_perform_remote_write do |unlocked_collection|
  ...
  def destroy
    collection = Collection.find(params[:id])
    authorize collection
    collection.flush_cache_and_perform_remote_write do |uo|
      if uo.destroy
